...
Provide the Ashita SDK directory, the MinHook source directory, and the
`capstone-5.0.9/src` directory when configuring. CMake builds Capstone and
MinHook as static dependencies. Python generates the native discovery constants
from `tools/discovery_rules.py`; both that schema and
`tools/generate_native_rules.py` are required build inputs.
...
The resulting plugin is `build/Release/limitbreak.dll`. A local development
checkout may also contain an untracked `tests/` directory. When it is present,
CMake adds the native test targets and they can be run with:

```powershell
ctest --test-dir build -C Release --output-on-failure
```

A fresh public clone does not need the local test suite to configure or build.
